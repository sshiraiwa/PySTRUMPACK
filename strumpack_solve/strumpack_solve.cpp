...
StrumpackSolver<T1, T2>::StrumpackSolver(int argc, char *argv[], bool verbose){
  spss=new STRUMPACK_SparseSolver();  
  STRUMPACK_init_mt(spss, T1, STRUMPACK_MT, argc, argv, verbose);
}

template <STRUMPACK_PRECISION T1, typename T2>    
StrumpackSolver<T1, T2>::StrumpackSolver(MPI_Comm comm, int argc, char *argv[], bool verbose){
  spss=new STRUMPACK_SparseSolver();    
